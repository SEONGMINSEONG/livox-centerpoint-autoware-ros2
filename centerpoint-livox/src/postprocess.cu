...
PostprocessCuda::PostprocessCuda( const float min_x_range,
                                  ...
                                  const std::vector<float>& score_thresh)
                                  ...
                                  ,nms_overlap_thresh_(nms_overlap_thresh)
{
  ...
  for (size_t i = 0; i < score_thresh.size() && i < 3; ++i) {
    score_thresh_[i] = score_thresh[i];
  }
  std::cout << "score_thresh: [" << score_thresh_[0] << ", " << score_thresh_[1]
            << ", " << score_thresh_[2] << "]" << std::endl;
                                        ...
                                        std::vector<Box> &out_detections)
{
  ...
  int mum_block = DIVUP(max_obj, kNumThreads);
  cudaDeviceSynchronize();
  BoxFilterKernel<<<mum_block,kNumThreads>>>(score_thresh,min_x_range_,min_y_range_,min_z_range_,
  ...
  } else if ((no_box_debug_count_++ % 20) == 0) {
    std::vector<float> host_scores(max_obj);
    std::vector<int> host_labels(max_obj);
    std::vector<float> host_boxes(max_obj * kBoxBlockSize);
    GPU_CHECK(cudaMemcpy(host_scores.data(), pred_scores, sizeof(float) * max_obj, cudaMemcpyDeviceToHost));
    GPU_CHECK(cudaMemcpy(host_labels.data(), pred_labels, sizeof(int) * max_obj, cudaMemcpyDeviceToHost));
    GPU_CHECK(cudaMemcpy(host_boxes.data(), pred_bbox, sizeof(float) * max_obj * kBoxBlockSize, cudaMemcpyDeviceToHost));

    int best_idx = -1;
    float best_score = -std::numeric_limits<float>::infinity();
    for (int i = 0; i < max_obj; ++i) {
      if (std::isfinite(host_scores[i]) && host_scores[i] > best_score) {
        best_score = host_scores[i];
        best_idx = i;
      }
    }

    if (best_idx >= 0) {
      const float x = host_boxes[best_idx * kBoxBlockSize + 0];
      const float y = host_boxes[best_idx * kBoxBlockSize + 1];
      const float z = host_boxes[best_idx * kBoxBlockSize + 2];
      const bool in_range = (x > min_x_range_ && x < max_x_range_ && y > min_y_range_ &&
                             y < max_y_range_ && z > min_z_range_ && z < max_z_range_);
      std::cout << "no boxes after filter. top_raw_score=" << best_score
                << " label=" << host_labels[best_idx]
                << " xyz=[" << x << ", " << y << ", " << z << "]"
                << " in_range=" << (in_range ? "true" : "false")
                << " thresh=[" << score_thresh_[0] << ", " << score_thresh_[1]
                << ", " << score_thresh_[2] << "]" << std::endl;
    }
  }
  GPU_CHECK(cudaFree(score_thresh));
}
